...
After `/plugin install massed-compute@massed-compute` (or `--plugin-dir`) and enable:

1. Plugin is **enabled** (it ships `defaultEnabled: false` because it talks to a paid account).
2. `/mcp` lists `plugin:massed-compute:massed-compute` (HTTP, `https://vm.massedcompute.com/api/mcp`). A separate user-level `massed-compute` stdio gateway may also exist — do not confuse the two.
3. Authenticate with `claude mcp login plugin:massed-compute:massed-compute` or the in-app `/mcp` login. Consent is browser OAuth (full access vs read-only). Claude Code handles DCR + PKCE. No Bearer header is bundled.
...
3. Ask the user to complete OAuth in the browser (`claude mcp login plugin:massed-compute:massed-compute` if `/mcp` shows Needs authentication). Consent is account-bound. Recommend **read-only** unless they need launch / restart / terminate / SSH key changes.
4. After consent, call `account_token_validation` (empty args `{}`) on the **plugin** MCP tools, not a similarly named user-level server. Real account data means the connection works.
...
3. **Fallback only:** they may add a **separately named** Bearer server themselves (not via this plugin's `.mcp.json`, and not named `massed-compute` — that name collides with the plugin and the stdio gateway):

```
claude mcp add --transport http massed-compute-bearer https://vm.massedcompute.com/api/mcp --header "Authorization: Bearer <your-api-key>"
```
